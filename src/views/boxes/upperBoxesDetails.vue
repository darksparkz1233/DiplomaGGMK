<template>
  <div class="grid justify-center  xl:grid-rows-2 xl:grid-cols-2 lg:grid-rows-1 lg:grid-cols-2 font-raleway font-bold">
    ...
    <div class="flex w-full xl:break-words flex-col justify-center gap-6 xs:p-10">
      ...
      <p class="text-md">
        Высота: {{ queryProduct.width }}
        <br>
        Ширина: {{ queryProduct.height }}
        <br>
        Глубина:{{ queryProduct.depth }}
        <br>
        Каркас: {{ queryProduct.chasis }}
        <br>
        Фасад: {{ queryProduct.facade }}
        <br>
        Ручка: {{ queryProduct.knob }}
      </p>
...
onMounted(async () => {
  const result = await fetch(`http://localhost:3000/upperBoxes/${route.params.id}`)
